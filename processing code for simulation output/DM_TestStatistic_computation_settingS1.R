...
input_par <- expand.grid(input_rho0, input_mu0,
                         input_eps, input_sigma, input_rho, input_models, input_scores)
names(input_par) <- c("rho0", "mu0", "eps", "sigma", "rho", "model", "score")

df <- data.frame(input_par)
df$simID <- rep(1:450, length(input_models)*length(input_scores))

flist <- list.files("/path/to/Rdata-files/")
existing <- as.numeric(sapply(flist, FUN = function(x) as.numeric(strsplit(strsplit(x, "_settingS1_")[[1]][2], ".Rdata"))))
...
for(filename in flist){
  ...
  ID <- as.numeric(as.numeric(strsplit(strsplit(filename, "_settingS1_")[[1]][2], ".Rdata")))
...
save(df100, file = "/path/to/save/output/df_mvTNormal.Rdata")
