...
run_setting5 <- function(obsmodel, fcmodel, nout, ninit, nmembers, d, MCrep, rand_rep, progress_ind = FALSE, compute_crps, ...){
...
run_wrapper <- function(runID){
  sink(file = paste0(Rout_dir, "setting3B_", runID, ".Rout"))
  par_values <- as.numeric(input_par[ID, ])
  res <- run_setting5(obsmodel = 5, fcmodel = 5, nout = 1000, ninit = 500, nmembers = 50, 
                      ...
                      d = input_par$d[runID])
  savename <- paste0(Rdata_dir, "res_setting3B_", runID, ".Rdata")
